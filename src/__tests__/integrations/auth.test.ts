...
describe('Auth Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        await UserModel.deleteMany({
            $or: [{ email: testUser.email }, { fullName: testUser.fullName }]
        });
    });
    ...
    describe('POST /api/auth/register', () => {
        ...
        test('should not register with password mismatch', async () => {
            const response = await request(app)
                .post('/api/auth/register')
    ...
    describe('GET /api/auth/whoami', () => {
        ...
        test('should not get profile with malformed token', async () => {
            const response = await request(app)
                .get('/api/auth/whoami')
    ...
    describe('PUT /api/auth/update-profile', () => {
        ...
        test('should update profile with valid name only', async () => {
            const response = await request(app)
                .put('/api/auth/update-profile')
                .set('Authorization', `Bearer ${authToken}`)
                .send({ fullName: 'validupdatedname' });
            
            expect(response.status).toBe(200);
            expect(response.body).toHaveProperty('success', true);
            expect(response.body.data).toHaveProperty('fullName', 'validupdatedname');
        });
        ...
        test('should not update profile with empty name', async () => {
            const response = await request(app)
                .put('/api/auth/update-profile')
                .set('Authorization', `Bearer ${authToken}`)
                .send({ fullName: '' });
        ...
        test('should update profile with profile image', async () => {
            const response = await request(app)
                ...
                .field('fullName', 'userwithimage');
            
            expect(response.status).toBe(200);
            expect(response.body).toHaveProperty('success', true);
            expect(response.body.data).toHaveProperty('fullName', 'userwithimage');
        });
    });
});
